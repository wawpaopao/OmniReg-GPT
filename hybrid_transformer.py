...
class HierarchicalTransformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        ignore_index = 2,#need to emphasize 
        ...
        for dim, hierarchy, stride in zip(dims[:-1], hierarchies[:-1], hierarchical_stride[:-1]):
        ...
        self.post_token_emb_norms = mlist([nn.LayerNorm(dim) for dim in dims[:-1]])
        ...
        for _ in range(depth):
            ...
            for hierarchy, h_stride, h_dim, h_window_size, h_dim_head, h_heads, h_ff_mult in zip(hierarchies[:-1], hierarchical_stride[:-1], dims[:-1], window_sizes[:-1], dim_head[:-1], heads[:-1], ff_mult[:-1]):
            ...
            merge = HierarchicalMerge(
                dims = dims[:-1],
                dim_out = hierarchy_predict_dim if not self.hierarchy_merge_all else sum(dims[:-1]),
                h_strides = hierarchical_stride[:-1]
            )

            self.hierarchical_merges.append(merge)


        self.last_hierarchy_layer = HierarchicalBlock(
                        dim = dims[-1],
                        dim_head = dim_head[-1],
                        heads = heads[-1],
                        window_size = window_sizes[-1],
                        compress_factor = 1,
                        stride = hierarchical_stride[-1],
                        ff_mult = ff_mult[-1]
                    )

        self.last_merge = HierarchicalMerge(
            dims = dims,
    ...
    def generate(
        ...
        **kwargs
        ...
        for _ in range(seq_len):
            ...
            logits = self.forward(out, return_loss=False,return_hierarchical_embeds=False)[:, -1]
    ...
    def forward(
        ...
        for i, (layer, merge) in enumerate(zip_longest(self.layers, self.hierarchical_merges)):
        ...
        ##最后一层full_attention,做完attntion之后加入tokens，
        used_embedding = tokens[self.predict_hierarchy_index]
        
        full_attn_embedding_1= self.last_hierarchy_layer(used_embedding)
        full_attn_embedding = self.last_hierarchy_layer(full_attn_embedding_1)
        
        # tokens.append(used_embedding)
        tokens.append(full_attn_embedding)
        pooled = self.last_merge(tokens)
        predict_tokens = tokens[self.predict_hierarchy_index]
        predict_tokens = predict_tokens + pooled
        tokens[self.predict_hierarchy_index] = predict_tokens
